...
void easyWDM::UninitstallKeyHook()
{
    if (_hookKey)
    {
...
jsoncpp& easyWDM::get_config_item(jsoncpp& param)
{
    if (param.isObject())
    {
        return param;
...
bool easyWDM::initWDM()
{
    ...
    hotkey.forEach([&](std::string_view key, jsoncpp& obj)
    {
        bool result = SetHotkey(std::string(key), [this, &obj]()
        {
            //UninitstallKeyHook();
/*
            ...
            console.log("{:32} {}", psName, txtName);
            return true;*/

            //分解出对象,因为参数有可能是数组,能处理多个进程的
            auto& item = get_config_item(obj);
